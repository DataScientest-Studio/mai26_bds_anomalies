...
fig.compare_orig_encoded(images_train, pred_train, output_path, "images_reconstruites_train_good.png")
...
fig.histogramme_erreurs(np.concat([images_train_flat, images_test_good_flat, images_test_anomaly_flat], axis=0), 
                        np.concat([pred_train_flat, pred_test_good_flat, pred_test_anomaly_flat], axis=0), 
                        np.concat([np.zeros((len(images_train_flat))), np.zeros((len(images_test_good_flat))), np.ones((len(images_test_anomaly_flat)))], axis=0), 
                        threshold_mse, 
                        output_path, 
...
# ROC curve
fig.draw_roc_curve(np.concatenate([mse_train, mse_test_anomaly]), 
                   np.concatenate([np.zeros(len(mse_train)), np.ones(len(mse_test_anomaly))]), 
                   output_path, 
                   output_filename=f"roc_curve_{category}.png", 
                   category=category)

# Visualisation des images reconstruites pour les anomalies
fig.compare_orig_encoded(images_test_anomaly, pred_test_anomaly, output_path, "images_reconstruites_test_anomalies.png")
