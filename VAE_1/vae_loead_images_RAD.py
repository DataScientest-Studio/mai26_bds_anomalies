...
def load_liste_images(images_path, resized_dimension=(128,128), category='bottle', type='train', quality="good", include_augmented=True, limit_to=None):
    """
    ...
    - quality: "good" or "anomaly" or "both" (default "good")
    - include_augmented: whether to include augmented images (default True)
    ...
    images_path = images_path.joinpath(category, type)

    if not images_path.exists():
        raise FileNotFoundError(f"Le dossier {images_path} n'existe pas (vérifie la structure pour la catégorie '{category}')")

    # check all subdirectories in images_path for the specified category and type
    images_paths = []
    for subdir in images_path.iterdir():
        if subdir.is_dir() and (quality == "both" or subdir.name == quality or (subdir.name != "good" and quality == "anomaly")):
            images_paths.append(subdir)
    ...
    for images_path in images_paths:
        ...
        if augmented_path.exists() and include_augmented:
        ...
        if limit_to is None or limit_to <= 0:
            ...
            nb_images = limit_to - current_count
            current_count += len(fichiers)
